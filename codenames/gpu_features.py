...
from codenames.features import N_SCALARS, ROLE_ORDER, SENTINEL
...
def _batched_role_block(tensor_gpu: torch.Tensor, sims: SimilarityTensor, boards: list[Board], role: Role, space_idx: int, device: torch.device) -> torch.Tensor:
    ...
    elif max_words > pad_to:
        sorted_desc = sorted_desc[:, :, :pad_to]
    return sorted_desc
...
def build_features_batch_multi(sims: SimilarityTensor, boards: list[Board], turn_indices: list[int], device: torch.device) -> torch.Tensor:
    ...
    mask = torch.stack(masks).to(device)  # (n_boards, BOARD_SIZE)

    scalars = []
    for b, turn_index in zip(boards, turn_indices):
        own_remaining = float(b.remaining(Role.OWN))
        own_revealed = ROLE_COUNTS[Role.OWN] - b.remaining(Role.OWN)
        opponent_revealed = ROLE_COUNTS[Role.OPPONENT] - b.remaining(Role.OPPONENT)
    ...
    assert mask_b.shape[-1] == BOARD_SIZE and scalars_b.shape[-1] == N_SCALARS  # keep in sync with features.py's layout
