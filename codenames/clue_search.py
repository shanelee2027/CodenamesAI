...
Used by every spymaster (codenames/spymasters/), which all need to turn
a (n_clues,)-shaped score array into legal clue words.
"""
...
def mean_from_columns(columns: list[np.ndarray]) -> np.ndarray:
    ...
    caller doing many repeated lookups of the *same* board word (there
    are only ~400 possible ones) can cache the disk read once per word
